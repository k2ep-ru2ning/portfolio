...
- 협업 도구 내부의 (메신저, 캘린더와 같은) 여러 애플리케이션에서 **공통으로 사용하는 컴포넌트를 제공하는 라이브러리**를 **관리**
...
- 룸은 (회사에서 개발 중인 협업 도구에서) 유저들의 모임을 의미
- **서울시교육청**와 **한국농어촌공사**의 협업 도구에 적용
...
- 룸 관련 설정을 5개의 탭 형태로 구현
  - 프로필 탭: 룸 이름, 소개, 해시태그 등을 설정할 수 있는 폼 구현
  - 일반 탭: 룸 공개 여부, 최대 인원수 등을 설정할 수 있는 폼 구현
  - 멤버 탭: 멤버 목록 표시 및 멤버 역할 변경 기능 구현
  - 입장 탭: 내보낸 유저 관리 기능 구현, 입장 요청 허용 및 취소 기능 구현
  - 삭제 탭: 삭제 알림 다이얼로그 표시 및 삭제 기능 구현
- 현재 로그인한 유저의 룸에 대한 권한과 룸 타입을 바탕으로, 설정할 수 있는 옵션들만 표시하도록 구현

**성과**

- 폼 작성할 때, 반복적으로 작성해야 할 코드를 줄이기 위해 **React Hook Form 도입**

  - **폼 관련 코드의 일관성 증가 및 가독성 향상**

- **React Hook Form 사용할 때, 폼 상태 변경을 추적하지 못하는 문제 해결**
  - **\[상황\]**
    - `useForm` 함수가 제공하는 `formState` 객체는 `isDirty`, `isValid`, `isSubmitting` 등의 프로퍼티를 통해 현재 상태를 제공합니다.
    - 논리 연산자를 사용해 `formState.isDirty && formState.isValid`로 폼 제출 가능 여부를 확인했을 때, 폼의 변화가 있고 입력값이 유효해도 앞의 표현식이 `false`로 평가되는 문제가 발생했습니다.
  - **\[과정 & 결과\]**
    - 공식문서에서 논리 연산자와 `formState`를 함께 사용할 때, **`formState`의 특정 프로퍼티가 조건부로 참조되면 발생할 수 있는 문제**라고 소개하고 있었습니다.
    - `formState` 객체는 proxy로 한 번 감싸져 있는데, 조건부로 프로퍼티를 참조하면 proxy가 변화를 제대로 추적할 수 없어서 발생하는 문제라고 합니다.
    - 공식문서의 가이드를 참고해, 구조 분해 할당으로 **사용하고 싶은 프로퍼티를 명확하게 참조하고 나서 논리 연산자와 함께 사용해 문제를 해결했습니다.**
...
- Tmax WAPL
- 2023.08 - 2024.03
- **TypeScript, React, MobX, Emotion, MUI**
- 자영업 사장님들을 위한, "성공식당 커뮤니티" 프로젝트에 적용

**업무**

- 프로필 상태 관리를 위해 MobX Store Class 구조 구상
- 유저 프로필 구현
- 유저 프로필 편집 폼 구현
  - 프로필 이미지 프리뷰 기능 구현
  - 폼 상태 관리 로직을 재사용할 수 있도록 custom hook으로 분리
...
- 유저 프로필 컴포넌트 **리팩터링**
  ...
  - **\[상황\]**
    - 퇴사하신 분이 관리하던 유저 프로필 컴포넌트를 맡게 되었습니다.
    - 하나의 유저 프로필 컴포넌트가 상황에 따라 `4` 개의 다른 화면을 표시했습니다.
    - 컴포넌트 내부에는 복잡한 분기 처리가 많았고, Props의 개수도 많았습니다.
    - **거대한 하나의 컴포넌트를 그대로 두면 관리하기 힘들 거 같아서, 작은 컴포넌트로 나누기로 결정했습니다.**
  - **\[과정\]**
    - 하나의 거대한 컴포넌트를 역할과 화면에 따라 `4` 개의 컴포넌트로 분리했습니다.
    - "(데스크탑 용) 내 프로필", "(데스크탑 용) 다른 유저 프로필", "(모바일 용) 내 프로필", "(모바일 용) 다른 유저 프로필"로 분리했습니다.
  - **\[결과\]**
    - 변경해야 할 코드의 범위가 명확해져서, **문제가 발생하거나 요구사항이 변경되어도 빠르게 대응할 수 있었습니다.**
    - 컴포넌트가 가지는 Props의 개수가 적어져, 전달된 Props에 따른 컴포넌트의 동작을 예측하기 쉬워졌습니다.

- 프로필 정보 업데이트 API와 프로필 이미지 업데이트 API를 하나로 합치는 제안으로, **명확한 예외 처리 코드 작성**
  - **\[상황\]**
    - (**기존**에) **하나의 프로필 편집 폼을 제출하는 데 두 개의 API 호출을 하도록** 코드가 작성되어 있었습니다.
    - 폼을 제출하면, 닉네임과 상태 메시지를 업데이트하는 API와 프로필 이미지를 업데이트하는 API가 함께 호출되었습니다.
    - **두 API 요청 중 하나만 실패한 경우처럼 예외 처리하기 애매한 상황이 생겼습니다.**
  - **\[과정 & 결과\]**
    - 백엔드 개발자와 소통하며 하나의 API로 만들 수 있는지 검토했습니다.
    - 하나로 합쳐진 API를 사용하도록 코드를 수정했습니다.
    - 예외 처리 코드도 추가했습니다. 여러 API를 호출하고 각각의 API의 호출 결과에 따라 예외 처리를 하는 대신, **하나의 API 호출에 대해 예외 처리를 하게 되어 코드가 간결해졌습니다.**
  - **\[회고\]**
    - 하나의 폼 제출을 여러 API를 조합해서 호출하도록 처리했을 때, 어떤 불편함이 발생할 수 있는지 생각해 볼 수 있는 기회였습니다.
    - 많이 복잡해지더라도 기존 방식을 유지하면서 예외 처리를 할 수도 있었지만, **폼 안에 있는 필드 하나라도 업데이트에 실패한다면 폼 제출 실패로 간주하는 것이 더 타당하다고 생각해 하나의 API로 묶는 것을 제안했습니다.**
